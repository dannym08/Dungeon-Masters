...
Inferring from the graphs, one can notice the steady decline in the number of steps as deep-Q learning decides to take the best-policy path to the goal over time. Also observe the reward consistently remaining positive as the best-policy path is realized.
